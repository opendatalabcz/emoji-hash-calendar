...
class PriorityTransformer(EmojiTransformer):
    ...
    def transform(self, text: str) -> str:
        # 1️⃣ najdi všechny emoji podle user_mapping
        user_emojis = []
        ...
        # 2️⃣ získat emoji z base transformeru
        base_emojis_str = self.base.transform(text)
        # pokud base vrací více emoji oddělených mezerou
        base_emojis = base_emojis_str.split() if base_emojis_str != "❓" else []

        # 3️⃣ sloučit uživatelské a base emoji, odstranit duplicity
        all_emojis = user_emojis + [e for e in base_emojis if e not in user_emojis]

        if not all_emojis:
            return "❓"

        # 4️⃣ vrátit jako string oddělený mezerou
